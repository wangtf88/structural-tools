...
# Helper for file dialogs (hidden root)
def get_file_path(action='save', default_ext='.json'):
    root = tk.Tk()
    root.withdraw()
    root.wm_attributes('-topmost', 1)
    if action == 'save':
        file_path = filedialog.asksaveasfilename(defaultextension=default_ext, filetypes=[("JSON Files", "*.json")])
    else:
...
def main():
    ...
    with st.sidebar:
        ...
        if c_io1.button("💾 Save Config"):
            path = get_file_path('save')
            if path:
                # Filter session state to only widget keys
    ...
    with tab2:
        ...
        with c_p1:
            save_path = st.text_input("Export Folder Path", value=os.getcwd())
            if st.button("Save Report (HTML)", type="primary"):
                 try:
                     fname = f"Wall_Design_Report_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.html"
