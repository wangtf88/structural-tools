...
def main():
    ...
    with st.sidebar:
        ...
        if HAS_TKINTER:
            # Save Project (Unified - Local System Dialog)
            if st.button("💾 Save Project"):
             ...
             # Fallback for Server/Cloud Environment
             st.download_button(
                label="💾 Save Project",
                data=save_state(),
                file_name="project_v1.json",
                mime="application/json",
            )
             st.caption("Server mode: functionality limited to browser download.")
